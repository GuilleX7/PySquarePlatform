...
import resManager
from os import path
from states import state, menustate, mainstate
...
class LoadState(state.State):
    def create(self):
        ...
        self.title.center()
        self.instructions = text.Text("ARROWS to select, ENTER to choose, F5 to refresh files, ESC to return", resManager.loadSystemFont(None, 20), [0, 400])
